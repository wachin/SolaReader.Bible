...
class SolaReaderApp(QMainWindow):
    ...
    def open_database(self):
        ...
        if not path.exists():
            # Si no existe la ruta por defecto, buscar en la carpeta data
            script_dir = Path(__file__).parent
            default_db = script_dir / "data" / "KJV+.SQLite3"
            ...
            if default_db.exists():
                ...
                # Si tampoco existe en data, abrir diálogo
                path, _ = QFileDialog.getOpenFileName(
                    self, "Seleccionar traducción", "", "SQLite Files (*.sqlite *.db *.sqlite3)"
                )
                if not path:
                    self.status_bar.showMessage("❌ No se seleccionó ninguna base de datos")
                    return
        ...
        try:
            ...
            if not self.db_path.exists():
                raise FileNotFoundError(f"No se encontró el archivo: {self.db_path}")
        ...
        try:
            if CONFIG_FILE.exists():
                with open(CONFIG_FILE, "r", encoding="utf-8") as f:
                    config = json.load(f)
                # Si existe configuración guardada, usarla
                ...
                if saved_translation and Path(saved_translation).exists():
                    ...
                    # Si la ruta guardada no existe, usar la por defecto
                    script_dir = Path(__file__).parent
                    default_db = script_dir / "data" / "KJV+.SQLite3"
                    self.current_translation = str(default_db)
